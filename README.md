...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create jotdown-v2-7a96
  fly apps create jotdown-v2-7a96-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app jotdown-v2-7a96
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app jotdown-v2-7a96-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'jotdown-v2-7a96-staging' does not match app name in config file 'jotdown-v2-7a96'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name jotdown-v2-7a96-db
  fly postgres attach --app jotdown-v2-7a96 jotdown-v2-7a96-db

  fly postgres create --name jotdown-v2-7a96-staging-db
  fly postgres attach --app jotdown-v2-7a96-staging jotdown-v2-7a96-staging-db
  ```
